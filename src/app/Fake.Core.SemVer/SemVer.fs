...
type SemVerInfo =
    ...
    override x.GetHashCode() =
        hash (x.Major, x.Minor, x.Patch, x.Build, x.PreRelease)
...
module SemVer =
    ...
    let isValid version =
        match version with
        | SemVer [ ValidVersion major; ValidVersion minor; ValidVersion patch; pre; build ] -> true
        | _ -> false
        ...
        try
            ...
            let prerelease =
                if suffix.IsEmpty || suffix.Tail.IsEmpty then
                    ...
                    String.Concat(suffix.Tail).TrimEnd([| '.'; '-' |])
