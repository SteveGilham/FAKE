...
and [<NoComparison; NoEquality>] TargetContext =
    ...
    member x.HasError = x.PreviousTargets |> List.exists (fun t -> t.Error.IsSome)
...
//#nowarn 44
    ...
    member x.Wrap() =
        match x.info with
        | Some info -> BuildFailedException(info, x.Message, x :> exn)
        | None -> BuildFailedException(x.Message, x :> exn)
...
module Target =
    ...
    let internal getNormalizedFileName (fileName: string) =
        ...
        if isWindows then
            if isNull fn then null else fn.ToLowerInvariant()
        else
            fn
        ...
        if shouldCollectStack () then
            ...
            let fn =
                ...
                |> Option.bind (fun ass ->
                    ...
                    |> Seq.tryFind (fun (fn, sf) ->
                        ...
                        && sf.GetMethod().DeclaringType.Assembly.FullName = fullName))
                ...
                |> Option.orElseWith (fun _ ->
                    ...
                    |> Seq.tryFind (fun (fn, _) ->
                        ...
                        not (String.IsNullOrEmpty fn)
                        && (fn.EndsWith ".fsx" || scriptName = normalizedScriptFile)))
            ...
            |> Option.defaultWith (fun _ ->
                ...
                { File = null
                  Line = 0
                  Column = 0
                  ErrorDetail = sprintf "stackframe not found, available:%s" framesString })
        else
            { File = null
              Line = 0
              Column = 0
              ErrorDetail = "not requested" }
    ...
    let internal getVarWithInit name f =
        ...
        let getVar, _, setVar = FakeVar.define varName
    ...
    /// </summary>
    let internal getAllTargetsNames () =
        getTargetDict () |> Seq.map (fun t -> t.Key) |> Seq.toList
    ...
    let internal getInternal name : InternalTarget =
        ...
        | _ ->
            Trace.traceError
            <| sprintf "Target \"%s\" is not defined. Existing targets:" name
    ...
    let get name : Target = (getInternal name).AsTarget
    ...
    let internal runSimpleContextInternal
        (traceStart: string -> string -> string -> Trace.ISafeDisposable)
        context
        (target: Target)
        =
            ...
            getTargetDict().[targetName] <-
                { target with
                    ...
                    SoftDependencies =
                        target.SoftDependencies
                        |> List.filter (fun d ->
                            not (String.Equals(d.Name, dependentTargetName, StringComparison.OrdinalIgnoreCase))) }
    ...
    /// </summary>
    let internal dependency targetName dependentTargetName =
        dependencyAtFront targetName dependentTargetName

    /// <summary>
    /// Adds the dependency to the list of soft dependencies.
    /// </summary>
    let internal softDependency targetName dependentTargetName =
        softDependencyAtFront targetName dependentTargetName

    /// <summary>
    /// Adds the dependencies to the list of dependencies.
    /// </summary>
    let internal Dependencies targetName dependentTargetNames =
        dependentTargetNames |> List.iter (dependency targetName)

    /// <summary>
    /// Adds the dependencies to the list of soft dependencies.
    /// </summary>
    let internal SoftDependencies targetName dependentTargetNames =
    ...
    let internal writeInfoFile file =
        ...
        let createDeclJson (decl: DeclarationInfo) =
            sprintf
                "{ \"file\": %s, \"line\": %d, \"column\": %d, \"errorDetail\": %s }"
        ...
        let createDepJson (dep: Dependency) =
            sprintf "{ \"name\": %s, \"declaration\": %s }" (createJsonString dep.Name) (createDeclJson dep.Declaration)

        let joinJsonObjects createObj (objs: _ seq) =
            objs |> Seq.map createObj |> (fun s -> "[ " + String.Join(", ", s) + " ]")
        ...
        |> fun targets -> System.IO.File.WriteAllText(file, sprintf "{ \"targets\": %s }" targets)
    ...
    let private visitDependencies repeatVisit fVisit targetName =
        let visit fGetDependencies fVisit targetName =
            ...
            let rec visitDependenciesAux orderedTargets =
                function
                    ...
                    | added when added || repeatVisit ->
                        ...
                        let newLeft =
                            (fGetDependencies target
                             |> Seq.map (fun (depType, targetName) -> (level + 1, depType, targetName))
                             |> Seq.toList)
                            @ workLeft

                        let newOrdered =
                            if added then
                                (targetName :: orderedTargets)
                            else
                                orderedTargets

                        visitDependenciesAux newOrdered newLeft
                    | _ -> visitDependenciesAux orderedTargets workLeft
                | _ -> orderedTargets
        ...
        let visited, _ =
            visit (fun t -> t.Dependencies |> List.rev |> withDependencyType DependencyType.Hard) ignore targetName
        ...
        if executedTargets.Length > 0 then
            ...
            let alignedString (name: string) duration extra =
                ...
                if (String.IsNullOrEmpty extra) then
                    ...
                    sprintf "%s   %s   (%s)" (name.PadRight width) (durString.PadRight "00:00:00.0000824".Length) extra

            let aligned (name: string) duration extra =
                alignedString name duration extra |> Trace.trace

            let alignedWarn (name: string) duration extra =
                alignedString name duration extra |> Trace.traceFAKE "%s"

            let alignedError (name: string) duration extra =
                alignedString name duration extra |> Trace.traceError
    ...
    let internal determineBuildOrder (target: string) =
        ...
        let rec visitDependenciesAux =
            function
            // NOTE: should be tail recursive
            | targetName :: workLeft ->
                if visited.Add(targetName) then
                    let target = get targetName
        ...
        let rec findOrder progress (targetLeft: Target list) =
            ...
            if List.isEmpty execute then
                failwithf "Could not progress build order in %A" targetLeft
            ...
            if List.isEmpty left then
                ...
                |> Seq.iter (targetLeftSet.Remove >> ignore)
    ...
    module internal ParallelRunner =
        ...
        let createCtxMgr (order: Target[] list) (ctx: TargetContext) =
            let body (inbox: MailboxProcessor<RunnerHelper>) =
                async {
                    let targetCount = order |> Seq.sumBy (fun t -> t.Length)

                    let resolution =
                        Set.ofSeq (order |> Seq.concat |> Seq.map (fun t -> String.toLower t.Name))
                            ...
                            | GetNextTarget (newCtx, reply) ->
                                ...
                                if known.Count = targetCount then
                                    ...
                                    let calculateOpenTargets () =
                                        let isOpen (t: Target) =
                                            not (known.ContainsKey(String.toLower t.Name))
                                            && // not already finised
                                            not (
                                                runningTasks
                                                |> Seq.exists (fun r -> String.toLower r.Name = String.toLower t.Name)
                                            ) // not already running

                                        order |> Seq.concat |> Seq.filter isOpen
                                    ...
                                    let runnable =
                                        ...
                                        calculateOpenTargets () |> Seq.filter isRunnable |> Seq.toList
                                    ...
                                    | None ->
                                        if runningTasks.Length = 0 && resolution.Count > known.Count then
                                            ...
                                            failwithf
                                                "Error detected in fake scheduler: resolution '%s', known '%s'"
                                                resolutionStr
                                                knownStr
                                        // queue work
                                        let tcs = TaskCompletionSource<TargetContext * Target option>()
                                        ...
                                        let running =
                                            String.Join(", ", runningTasks |> Seq.map (fun t -> sprintf "'%s'" t.Name))
                                        ...
                                        let orderedOpen =
                                            let isDependencyResolved = String.toLower >> known.ContainsKey

                                            let isDependencyRunning t =
                                                runningTasks
                                                |> Seq.exists (fun running -> String.toLower running.Name = t)

                                            let isDependencyResolvedOrRunning t =
                                                isDependencyResolved t || isDependencyRunning t
                                        ...
                                        let openList =
                                            String.Join(
                                                ", ",
                                                orderedOpen :> seq<_>
                                                |> (if orderedOpen.Length > 3 then Seq.take 3 else id)
                                                |> Seq.map (fun t -> sprintf "'%s'" t.Name)
                                            )
                                            + (if orderedOpen.Length > 3 then ", ..." else "")

                                        Trace.tracefn
                                            "FAKE worker idle because '%d' targets (%s) are still running and all ('%d') open targets (%s) depend on those. You might improve performance by splitting targets or removing dependencies."
                                            runningTasks.Length
                                            running
                                            openTargets.Length
                                            openList
    ...
    let internal runInternal singleTarget parallelJobs targetName args =
        ...
        | Some d ->
            failwithf
                "You set a task description (%A) but didn't specify a task. Make sure to set the Description above the Target."
                d
        | None -> ()
        ...
        let context =
            let captureContext (f: 'a -> unit) =
                ...
                (fun a ->
                    ...
                    if ctx <> nctx then
                        Context.setExecutionContext ctx
            ...
            use __ =
                Console.CancelKeyPress |> Observable.first |> Observable.subscribe cancelHandler
            ...
            let context =
                ...
                if parallelJobs > 1 && not singleTarget then
                    ...
                    if singleTarget then
                        ...
                        targets
                        |> Array.fold (fun context target -> runSingleTarget target context) context
    ...
    let internal getBuildFailedException (context: TargetContext) =
        let targets =
            ...
            |> Seq.map (fun (_er, target) -> target.Name)
            |> Seq.distinct
        ...
        let inner =
            AggregateException(AggregateException().Message, context.ErrorTargets |> Seq.map fst)
    ...
    let updateBuildStatus (context: OptionalTargetContext) =
        ...
        | Some c when c.HasError ->
            let targets =
                c.ErrorTargets |> Seq.map (fun (_er, target) -> target.Name) |> Seq.distinct
    ...
    let raiseIfError (context: OptionalTargetContext) =
        ...
        then
            getBuildFailedException c.Value |> raise
    ...
    let internal parseArgsAndSetEnvironment () =
        ...
        let trySplitEnvArg (arg: string) =
            ...
            if idx < 0 then
                Trace.traceError (
                    sprintf "Argument for -e should contain '=' but was '%s', the argument will be ignored." arg
                )
        ...
        | Choice1Of2 results ->
            let envs =
                ...
                | Some args -> args |> List.choose trySplitEnvArg
                | None -> []

            for key, value in envs do
                Environment.setEnvironVar key value
            ...
            if DocoptResult.hasFlag "--list" results then
                ...
                let parallelJobs =
                    ...
                    | None -> Environment.environVarOrDefault "parallel-jobs" "1" |> int
    ...
    module WithContext =
        ...
        let run parallelJobs targetName args =
            runInternal false parallelJobs targetName args |> OptionalTargetContext.Set
    ...
    /// </remarks>
    let initEnvironment () = initAndProcess ignore
